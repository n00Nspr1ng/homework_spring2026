...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        # for _ in range(len(rewards)):
            # discounted_return.append(sum([self.gamma ** t_prime * rewards[t_prime] for t_prime in range(len(rewards))]))
        
        total_return = sum((self.gamma**t)*r for t, r in enumerate(rewards))
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        #     discounted_reward_to_go.append(sum([self.gamma ** (t_prime - t) * rewards[t_prime] for t_prime in range(t, len(rewards))]))
        
        discounted_reward_to_go = np.zeros(len(rewards), dtype=np.float32)
        r = 0.0
        for t in reversed(range(len(rewards))):
            r = rewards[t] + self.gamma * r
            discounted_reward_to_go[t] = r
